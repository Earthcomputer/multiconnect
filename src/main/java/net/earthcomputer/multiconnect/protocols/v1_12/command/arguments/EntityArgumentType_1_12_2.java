...
public final class EntityArgumentType_1_12_2 implements ArgumentType<Void> {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        ...
        if ((reader.canRead() && reader.peek() == '@') || !suggestPlayerNames) {
            ...
            playerCompletions =
                    ((SharedSuggestionProvider) context.getSource()).customSuggestion(context);
        }
    ...
    private static class EntitySelectorParser {
        ...
        private void suggestOption() {
            ...
            suggestor = builder -> {
                ...
                SharedSuggestionProvider.suggest(SELECTOR_OPTIONS.keySet().stream()
                ...
                var scoreOptions = getScoreObjectives().thenCompose(objectives -> {
                    SharedSuggestionProvider.suggest(objectives.stream()
                                    .map(str -> "score_" + str)
                ...
                parser.suggestor = builder -> {
                    ...
                    if (parser.playersOnly) {
                        if (ConnectionInfo.protocolVersion <= Protocols.V1_10) {
                            SharedSuggestionProvider.suggest(new String[] {"Player"}, builder);
                        } else {
                            SharedSuggestionProvider.suggestResource(Collections.singleton(new ResourceLocation("player")), builder);
                        }
                    } else {
                        if (ConnectionInfo.protocolVersion <= Protocols.V1_10) {
